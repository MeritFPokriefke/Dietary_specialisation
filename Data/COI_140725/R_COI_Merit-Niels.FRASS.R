...
#################################################################################################################
data.map$year <- as.character(data.map$year)

data.map$plot <- substr(data.map$collector_nr, 3, 3)
...
#ggplot(controls.melt.rel, aes(x=species, y=Abundance, col=Year))+geom_boxplot()+scale_y_log10()+ 
...
#data.species <- subset_samples(data.species, !(year %in% control_samples)) # woran metadaten erkennen, also daher passt, filtern angepasst davor
...
filter_taxa.phylum = c("Rotifera", "Nematoda", "Platyhelminthes", "Bryozoa", "Cnidaria", "Echinodermata",
                       "Gastrotricha", "Porifera", "Tardigrada", "Nemertea", "Chordata", "Animalia spc")
filter_taxa.class = c("Actinopterygii", "Amphibia", "Bivalvia", "Ascidiacea", "Branchiopoda", "Aves", "Reptilia",
                      "Thecostraca", "Polyplacophora", "Polychaeta", "Malacostraca", "Arthropoda spc")
...
filter_taxa.species = c("Homo sapiens", "Meleagris gallopavo", "Cyornis spc 1", "Platorchestia platensis",
                        "Porcellanidae spc 1", "Goniobranchus spc 1", "Styliola spc 1",
                        "Zetela spc 1")
...
### nre filtering(include order level if needed)
...
sample_metadata1 <- sample_metadata[, c("ShannonIndex", "EffectiveRichness", "tube_ID")] # find identifier
...
data.order <- transform_sample_counts(data.order, function(x) x / sum(x)) #remove control samples
...
data.family <- transform_sample_counts(data.family, function(x) x / sum(x))#remove control samples
...
write.csv(sample_metadata3, 'D:/Pokriefke/Documents/Data_analysis/Chapter2/Git/DietVar/Data/Extracted/frass3_020925.csv',row.names = FALSE)



sample_metadata4 <- sample_metadata3[ , c("tube_ID", "tube_nr", "collection_date", "April.day", "collector_nr",
                                          "weight_.g.", "collection_time", "obs", "obs_weighing", "Drying.time",
                                          "comments", "year", "type", "plot",
...
final_sample_metadata1 <- merge(sample_metadata1, sample_metadata2, by = "tube_ID" )
final_sample_metadata2 <- merge(final_sample_metadata1, sample_metadata4, by = "tube_ID")


write.csv(final_sample_metadata2, 'D:/Pokriefke/Documents/Data_analysis/Chapter2/Git/DietVar/Data/Extracted/frass2_270825.csv',row.names = FALSE)
...
pdf("D:/Pokriefke/Documents/Data_analysis/Chapter2/Git/DietVar/Output/Plots/frass_plot_cum_rel_abundance_120825.pdf", width=7, height=8)


sample_data(data.species.rel.filter)$plot <- factor(sample_data(data.species.rel.filter)$plot)


my_colors <- c(
  "#E6B07C",  # light caramel
  "#C47F2E",  # burnt orange-brown
  "#A0522D",  # sienna
  "#8B5A2B",  # rust
  "#6F4E37",  # coffee
  "#5B3A29"  # raw umber                                 
...
plot_top_taxa_by_metadata(data.species.rel.filter, top_n = 50, metadata_var = "plot", group = "order",
                          fill_colors = my_colors)
dev.off()


pdf("D:/Pokriefke/Documents/Data_analysis/Chapter2/Git/DietVar/Output/Plots/frass_year_cum_rel_abundance_120825.pdf", width=7, height=8)

sample_data(data.species.rel.filter)$year <- factor(sample_data(data.species.rel.filter)$year)


my_colors <- c("#C7A27C", "#4B2E2B")

plot_top_taxa_by_metadata(data.species.rel.filter, top_n = 50, metadata_var = "year", group = "order",
                          fill_colors = my_colors)
