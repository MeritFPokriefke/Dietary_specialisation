...
plot_top_taxa_by_metadata <- function(ps_obj,
                                      top_n = 20,
                                      metadata_var = "Type",
                                      group = NULL,
                                      output_file = NULL,
                                      width = 10,
                                      height = 6,
                                      fill_colors = NULL) { # added this so I can make custom colors
  # --- Sample metadata ---
  meta_df <- as(sample_data(ps_obj), "data.frame")
  meta_df <- tibble::rownames_to_column(meta_df, var = "SampleID")
  meta_df$SampleID <- as.character(meta_df$SampleID)

  if (!(metadata_var %in% colnames(meta_df))) {
    stop(paste("Metadata variable", metadata_var, "not found in sample_data."))
  }

  # --- Abundance table ---
  abund_df <- as.data.frame(otu_table(ps_obj))
  if (taxa_are_rows(ps_obj)) {
    abund_df <- t(abund_df)
  }
  abund_df <- as.data.frame(abund_df)
  abund_df$SampleID <- as.character(rownames(abund_df))
  ...
  # --- Join metadata ---
  merged_df <- abund_long %>%
    left_join(meta_df, by = "SampleID")

  # --- Taxonomy table (if group is specified) ---
  if (!is.null(group)) {
    tax_df <- as(tax_table(ps_obj), "matrix") %>%
      as.data.frame() %>%
      tibble::rownames_to_column(var = "Taxa")
    
    if (!(group %in% colnames(tax_df))) {
      stop(paste("Group column", group, "not found in tax_table."))
    }

    # Join taxonomy info
    merged_df <- merged_df %>%
      left_join(tax_df[, c("Taxa", group)], by = "Taxa") %>%
      rename(Group = !!sym(group))  # standardize group name
  } else {
    merged_df$Group <- "All taxa"
  }

  # --- Get top N taxa across all samples ---
  top_taxa <- merged_df %>%
    group_by(Taxa) %>%
  ...
  plot_df <- filtered_df %>%
    group_by(Group, Taxa, !!sym(metadata_var)) %>%
    summarise(TotalAbundance = sum(Abundance), .groups = "drop")

  # --- Plot ---
p <- ggplot(plot_df, aes(x = reorder(Taxa, -TotalAbundance),
                        y = TotalAbundance,
                        fill = !!sym(metadata_var))) +
  geom_bar(stat = "identity", position = "stack", width = 0.8) +   # fix bar thickness here
  facet_grid(Group ~ ., space = "free", scales = "free_y")  +
  coord_flip() +
  theme_bw() +
  labs(x = "Taxa", y = "Cumulative Abundance", fill = metadata_var)+
  theme(strip.text.y = element_text(angle = 0, hjust = 0) )

if (!is.null(fill_colors)) { # added this so I can call upon my colors
...
build_pseudophylo_hclust <- function(physeq, method = "lv", linkage = "average") {
  # Extract taxonomy table
  tax_tab <- as.data.frame(tax_table(physeq))
  tax_tab[is.na(tax_tab)] <- "Unclassified"
  
  # Collapse taxonomy ranks into single strings
  tax_strings <- apply(tax_tab, 1, paste, collapse = ";")
  names(tax_strings) <- rownames(tax_tab)
  
  # Compute pairwise distances between taxonomy strings
  dist_mat <- stringdistmatrix(tax_strings, tax_strings, method = method)
  rownames(dist_mat) <- colnames(dist_mat) <- names(tax_strings)
  
  # Hierarchical clustering
  hc <- hclust(as.dist(dist_mat), method = linkage)
  
  # Convert to phylo object
  tree <- as.phylo(hc)
  tree$tip.label <- names(tax_strings)[hc$order]
  
  # Add to phyloseq
  physeq_out <- merge_phyloseq(physeq, tree)
  return(physeq_out)
}
