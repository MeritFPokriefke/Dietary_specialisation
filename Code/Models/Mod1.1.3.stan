data {
  ...
  int<lower=1> J;         // number of orders
  ...
  matrix[N_w, J] w;        // RRA birds, each column = order
  ...
  vector[N_w] LSC1;       // Type LSC1
  vector[N_w] LSC2;       // Type LSC2
...
parameters {
  ...
  matrix[J, 8] bw;                 // beta fixed effects
...
// Likelihood: zero-inflated beta hurdle RRA (w) across orders
for (j in 1:J) {                          // loop over orders
  for (i in 1:N_w) {
    ...
    // Order-specific zero-inflation probability
    real pi_i = pi_w[j];  
...
generated quantities {
  ...
  vector[J] V_e_w;        // residual variance (response scale)
  ...
  for (j in 1:J) {
    ...
    // Residual variance (response scale, zero-inflated Beta)
    real acc_var = 0;
    real acc_mu  = 0;
    for (i in 1:N_w) {
    ...
    // Treatment means on response scale
    ...
    // Interaction effect (departure from additive expectation)
