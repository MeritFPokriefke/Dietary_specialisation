...
### models are refitted for orders based on differences among bird types (adult, nestling)
...
###################################################################################################### A & C ----
### model with bird type of adult and nestling for both species (A,C; all)
...
orders <- c("Diptera", "Araneae" , "Coleoptera") # orders with bird type differences in adult and nestling
...
md <- stan("Code/Models/Mod1.2.stan", data = dfl, seed = 19978030496,
           chains = 4, iter = 3000,  warmup = 2000, 
           thin = 1, cores = 4, control = list(adapt_delta = 0.95))

### save output locally
saveRDS(md, file = "Output/Models/M2_all_diet_var_290825.rds")
...
#launch_shinystan(md) # check details
#dat_plot <- as.data.frame(round(summary(md)$summary[,c(1,4,6,8,9,10)],3))

## save output
write.csv(dat_zip, "Output/Models/M2_all_diet_var_290825.csv")
...
md <- stan("Code/Models/Mod1.1.2.stan", data = dfl, seed = 170564536,
           chains = 4, iter = 3000,  warmup = 2000, 
           thin = 1, cores = 4, control = list(adapt_delta = 0.95))

saveRDS(md, file = "Output/Models/M2_B_diet_var_290825.rds")
...
write.csv(dat_zip, "Output/Models/M2_B_diet_var_290825.csv")
...
orders <- c("Lepidoptera", "Diptera", "Araneae", "Coleoptera") # orders with bird type differences in adult and nestling
...
md <- stan("Code/Models/Mod1.1.2.stan", data = dfl, seed = 1751610516,
           chains = 4, iter = 3000,  warmup = 2000, 
           thin = 1, cores = 4, control = list(adapt_delta = 0.95))

saveRDS(md, file = "Output/Models/M2_G_diet_var_290825.rds")
...
write.csv(dat_zip, "Output/Models/M2_G_diet_var_290825.csv")
