...
######################################################
### Script: 07#M_Weight_Diet_C2

### Prediction 3: Fledging mass and dietary specialisation
...
df1 <- read.csv("Data/Extracted/weight_sum_d15_220626.csv", header=TRUE)
...
df_w <- df1 %>%
  filter(!is.na(sumweight), Species == "B") %>%# take out 4 observations where no weight data, choose blue tits
...
md <- stan("Code/Models/Mod7.stan", data = dfl, seed = 1374957732,
           chains = 4, iter = 5000,  warmup = 2000, 
           thin = 1, cores = 4, control = list(adapt_delta = 0.99))

### save output locally
saveRDS(md, file = "Output/Models/M_B_Weight_Diet_230626.rds")
...
### save output
write.csv(dat_zip, "Output/Models/M_B_Weight_Diet_230626.csv")
...
df_z <- df_z[!is.na(df_z$propp1_G), ]  # 14 obs with no estimate excluded
df_w <- df1 %>%
  filter(!is.na(sumweight), Species == "G") %>%# take out 4 observations where no weight data, choose blue tits
  distinct(BroodID, .keep_all = TRUE) # create fledging success data frame for nestlings
...
### fit bayesian model using STAN
md <- stan("Code/Models/Mod7.stan", data = dfl, seed = 267296137,
           chains = 4, iter = 5000,  warmup = 3000, 
           thin = 1, cores = 4, control = list(adapt_delta = 0.99)) # checked EES and is fine (due to mean(w) use in generated quantities, is a constant though)

### save output locally
saveRDS(md, file = "Output/Models/M_G_Weight_Diet_230626.rds")
...
#rstan::get_seed(md) # model seed 267296137
...
### save output
write.csv(dat_zip, "Output/Models/M_G_Weight_Diet_230626.csv")
