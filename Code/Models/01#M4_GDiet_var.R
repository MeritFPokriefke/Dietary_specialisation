...
### models are refitted for orders based on differences among bird types (no differences between types)
...
orders <- c("Lepidoptera") # orders with no bird type differences
...
md <- stan("Code/Models/Mod1.4.stan", data = dfl, seed = 9589054,
           chains = 4, iter = 3000,  warmup = 2000, 
           thin = 1, cores = 4, control = list(adapt_delta = 0.95))

### save output locally
saveRDS(md, file = "Output/Models/M4_all_diet_var_090925.rds")
...
#launch_shinystan(md) # output checks
#dat_plot <- as.data.frame(round(summary(md)$summary[,c(1,4,6,8,9,10)],3))

## save output
write.csv(dat_zip, "Output/Models/M4_all_diet_var_090925.csv")
...
orders <- c("Hymenoptera") # orders with no bird type differences
...
md <- stan("Code/Models/Mod1.1.4.stan", data = dfl, seed = 9589054,
           chains = 4, iter = 3000,  warmup = 2000, 
           thin = 1, cores = 4, control = list(adapt_delta = 0.95))

saveRDS(md, file = "Output/Models/M4_G_diet_var_090925.rds")
...
write.csv(dat_zip, "Output/Models/M4_G_diet_var_090925.csv")
